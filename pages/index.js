...
Home.getInitialProps = async function () {
  ...
  if (endpoint) {
    ...
    const querySpec = {
      ...
      " d.val,"+
      " d.placeholder"+
      " from c join f in c.forms join s in f.submissions join d in s.data",
    };
...
export default function Home({ CosmoData }) {
  return (
    <div>
      {CosmoData.map(({accName, email, address,title, timestamp, placeholder, val}) => (
        <div key={accName}>
          <p>User: {accName} - {email}</p>
          <p>Address: {address}</p>
          <p>Forms:</p>
          <div key={title}>
          <p> - {title} - submissions: </p>
            <div key={timestamp}>
              <p>{timestamp}:</p>
              <div key={val}>
                <p>{placeholder}= {val}</p>
